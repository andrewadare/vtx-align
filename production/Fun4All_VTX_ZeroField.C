...
void Fun4All_VTX_ZeroField( const char *inputfile = "/phenix/u/theok/hhj2/PRDF/ZEROFDATA_P00-0000411768-0000.PRDFF")
{
  ...
  for (int i = 0; i < kMAXSIGNALS; i++)
    ...
    svxpar->set_BeamCenter((0.348+0.367)/2.0,(0.093+0.062)/2.0);
    //offsets for Taebong svxPisa.par_v18-2
    //svxpar->set_OffsetVtxToCnt(-0.409232,0.0168409,0.0);
    //svxpar->set_OffsetEastToWest(0.054877,-0.0141701,-0.037338);
    
    //offset for taebong
    /*
    svxpar->set_OffsetVtxToCnt(-0.409232,0.0168409,0.0);
    svxpar->set_OffsetEastToWest(0.003539,-0.0159371,0.0);*/

    svxpar->set_OffsetVtxToCnt(0.0,0.0,0.0);
    svxpar->set_OffsetEastToWest(0.367-0.348,0.062-0.093,0.0);
    svxpar->set_ReadGeoParFromFile(1);
    //svxpar->set_GeometryFileName("svxPISA-411768.par.1");
    svxpar->set_GeometryFileName("svxPISA-411768.par.2");
...
*/
    ...
    for (int i = 0; i < trgsel.size(); i++)
    {
        ...
        sprintf(dstname, "v3_DST_SVX_%s", trgsel[i].c_str());
